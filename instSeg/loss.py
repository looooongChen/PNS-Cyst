...
def masked_cosine(y_true, y_pred, mask):
    '''
    Args:
        y_true: label map of size B x H x W x C
        y_pred: feature map of size B x H x W x C
    ...
    cos = tf.reduce_sum(y_true * y_pred, axis=-1, keepdims=True)
    cos = cos[mask>0]
    cos = tf.reduce_mean(v)
    return cos
...
def cosine_embedding_loss(y_true, y_pred, adj_indicator, max_obj, include_background=True):
    ...
    def _loss(x):
        ...
        # loss_inter = tf.sqrt(tf.reduce_sum(mu_interleave * mu_rep, axis=-1)**2)
        # loss_inter = tf.sqrt(tf.reduce_sum(mu_interleave * mu_rep, axis=-1)**2)
        loss_inter = tf.reduce_sum(mu_interleave * mu_rep, axis=-1) ** 2
        # loss_inter = tf.maximum(1.414-tf.norm(mu_interleave - mu_rep, ord=2, axis=-1),0)
...
# def cosine_l1_embedding_loss(y_true, y_pred, adj_indicator, max_obj, weight=0.01, include_background=True):
...
#     losses = tf.map_fn(_loss, (y_true, adj_indicator, y_pred))[2]
#     loss_sparse = tf.norm(y_pred, ord=1, axis=-1)
#     return tf.reduce_mean(losses) + weight * tf.reduce_mean(loss_sparse)
